...
data = pd.read_csv(r'Rumors01.csv')

for i in data.columns:
    print('%s的分布:\n' % i, data[i].value_counts())
    print('---------------------')

print('数据量：', len(data['rumorType']))
print('***********************')
print(data.info())
...
train_xx = np.reshape(a, (227,1,6901))
test_xx = np.reshape(b, (57, 1,6901))
...
history = model.fit(train_xx, train_y, batch_size=32, epochs=100, callbacks=[cp_callback],
